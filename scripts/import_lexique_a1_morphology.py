...
def main():
 ...
 db.commit(); created=excluded=analyses=0; db.execute('BEGIN')
 try:
  ...
  for form in db.execute("""SELECT form.id AS form_id, paradigm.target_object_id AS paradigm_id
   ...
   db.execute("INSERT OR IGNORE INTO relationships(id,source_object_id,target_object_id,relationship_type_code,source_kind,confidence) VALUES (?,?,?,?, 'curated',1.0)",(stable('relationship',f"{form['form_id']}|member_of_paradigm|{form['paradigm_id']}"),form['form_id'],form['paradigm_id'],'member_of_paradigm'))
  db.commit()
 except: db.rollback(); raise
 report={'release_id':release['id'],'sha256':sha(a.source),'form_analyses_imported':analyses,'explicitly_excluded_source_rows':excluded,'a1_verb_lemmas_with_source_forms':261}
