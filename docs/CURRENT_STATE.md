...
- Contextual AI Learning Assistance: the provider-neutral Learning Resource Resolver automatically completes visible missing learning slots with locally cached, clearly labelled AI drafts when online assistance is available. Resource identity includes target, kind, context, and language; English and enabled reference languages resolve, cache, label, and revise independently. Local graph analysis always renders first; sentence guidance is anchored after deterministic analysis; expanded follow-up resources generate progressively; and drafts never have a canonical graph write path. Gemini is the first development adapter, served through a local environment-key server.
...
- The combined, hash-locked Kaikki English Wiktionary and complementary French Wiktionary-derived releases currently supply source-backed senses for 8,658 of 8,767 A1–B2 word/POS objects (98.76%). This includes 17,729 lexical-sense objects, 17,657 independently evidenced English-gloss facts, and 270 independently evidenced French source-definition facts. All 8,767 lexical identities remain present and locally resolvable; 109 identities have no exact imported source-backed sense and are retained as explicit coverage gaps rather than filled with generated canonical content.
...
- Source coverage gaps remain explicit rather than fabricated: `bienvenir` has no Lexique form coverage; 109 A1–B2 word/POS identities currently lack an exact source-backed lexical sense; and 336 Kaikki phrase senses have an unresolved local A1–B2 component and are excluded from expression import.
